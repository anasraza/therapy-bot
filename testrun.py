...
class SetGetTest(ServiceTester):
    script = "app/scripts/wrapperservice.py"
    def run(self):
        time.sleep(1)
        
        WrapperService = self.service("WrapperService")
        WrapperService.set(0)
        assert WrapperService.get() == 0
        WrapperService.set(2)
        assert WrapperService.get() == 2
        return True
